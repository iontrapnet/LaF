...
require "gfx"
	local function loadPng(filename,depth)
		...
		local x; 
		if depth==1 then
			 x = torch.Tensor(surf.lcairo_object.width,surf.lcairo_object.height);         
			-- Black & White
		else 
			 x = torch.Tensor(surf.lcairo_object.width,surf.lcairo_object.height, depth);  
			-- RGB or RGBA
		end
		surf.lcairo_object:toTensor(x)
		return x;
	end
	...
	local function savePng(filename,x)
		local surf = gfx.ImageSurface(x:size(1),x:size(2))
	        surf.lcairo_object:fromTensor(x);
