...
class GP:
  def __init__(self,**settings):
    # gnuplot
    self.gp = Popen((os.path.join(settings.get('path') or '','gnuplot'),),stdout=PIPE,stdin=PIPE,close_fds=False)
    self.stdin,self.stdout = self.gp.stdin,self.gp.stdout

    # history
    hfile = settings.get('hfile',os.path.join(os.getenv('USERPROFILE'),'gp.py.hist'))
