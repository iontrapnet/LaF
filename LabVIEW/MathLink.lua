...
local header = io.open('ffi_mathlink.h')
...
function is_integer(x)
    return type(x) == 'cdata' and ffi.istype(int64,x)
end
...
local function to_string(self)
    ...
    elseif is_integer(self) then
        local r = tostring(self)
        if r:sub(-2,-1) == 'LL' then
            return r:sub(1,-3)
        else
            return r
        end
...
local function get_expr()
    ...
    --print('t = ',t)
    if t == ml.MLTKSYM then --35
        local bufp = ffi.new('const char*[1]')
        ml.MLGetSymbol(mlp, bufp)
        local r = ffi.string(bufp[0])
        ml.MLReleaseSymbol(mlp, bufp[0])
        return r
    elseif t == ml.MLTKSTR then --34
        local bufp = ffi.new('const char*[1]')
...
local function put_expr(expr)
    if is_expr(expr) then
        ...
        elseif t == 'cdata' and ffi.istype(int64,expr) then
...
function lua(expr)
    if #expr == 1 then
        ...
        if type(r) == 'string' and r:sub(1,1) == '"' then
            if not (r:match('^"[^{]+=') or r:match('^"\x1BL')) then
                r = '"return '..r:sub(2,-1)
            end
            if r:match('^"\x1BL') then
                r = Expr('load')(r)
            else
                r = Expr('load')(r)()
            end
...
if arg and #arg > 0 then
    ...
    answer()
    print(meval(meval(Expr('Factorial')(10))))
