...
function M.loadbuffer(L, buf, sz, chunkname)
	return check(L, C.luaL_loadbufferx(L, buf, sz, chunkname, ffi.NULL))
end
...
function M.loadfile(L, filename)
	return check(L, C.luaL_loadfilex(L, filename, ffi.NULL))
end
...
function M.openlibs(L, ...) --open specific libs (or all libs if no args given)
	local n = select('#', ...)
	if n == 0 then
		for k,v in pairs(lib_openers) do
            C.luaL_requiref(L, k, v, 1)
            M.pop(L, 1)
        end
    else
        for i=1,n do
            C.lua_pushcclosure(L, assert(lib_openers[select(i,...)]), 0)
            C.lua_callk(L, 0, 0, 0, ffi.NULL)
        end
    end
	return L
end
...
M.objlen = C.lua_rawlen
M.strlen = C.lua_rawlen
...
M.tonumberx = C.lua_tonumberx
M.tointegerx = C.lua_tointegerx
...
function M.get(L, index, opt)
	...
	elseif t == 'number' then
		return M.tonumberx(L, index, ffi.NULL)
...
function M.xpcall_opt(L, opt, errfunc, ...)
	...
	local ok, err = check(L, C.lua_pcallk(L, argc, C.LUA_MULTRET, errfunc, 0, ffi.NULL))
	if not ok then
		return false, err
	end
...
ffi.metatype('lua_State', {__index = {
	...
	tonumberx = M.tonumberx,
	tointegerx = M.tointegerx,
	...
	pcall = M.pcall,
	call = M.call,
	lua_pcallk = C.lua_pcallk,
	lua_callk = C.lua_callk,
	--gc
	gc = M.gc,
