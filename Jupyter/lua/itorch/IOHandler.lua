...
local io_stdo = ffi.C._open(arg[2], O_NONBLOCK)
...
function handleSTDO(ev)
   local nbytes = ffi.C._read(io_stdo,buffer,chunk_size)
   if nbytes > 0 then
...
function handleRawPub(sock)
   ...
   if m[1] == 'private_msg' then
      ...
      elseif m[2] == 'shutdown' then
         ...
         rawpub:close()
         heartbeat:close()
         ffi.C._close(io_stdo)
         os.execute('del ' .. arg[2]) -- cleanup files
         os.execute('del ' .. arg[3]) 
