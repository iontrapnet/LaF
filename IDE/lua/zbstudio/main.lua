...
require('lfs').chdir(debug.getinfo(1).source:gsub('/','\\'):match'@(.*)\\.*$' or io.popen'cd':read'*l')
...
local function setLuaPaths(mainpath, osname)
  ...
  ide.osclibs = package.cpath-- keep the list to use for various Lua versions
...
do
  ...
  if ide.osname == "Windows" and exepath:find("%.exe$") and not exepath:find("lua") then
    fullPath = exepath
