...
-- local console = ide.frame.bottomnotebook.shellbox
console:Connect(wxstc.wxEVT_STC_USERLISTSELECTION,
    function (event)
      if ide.wxver >= "2.9.5" and console:GetSelections() > 1 then
        local text = event:GetText()
        -- capture all positions as the selection may change
        local positions = {}
        for s = 0, console:GetSelections()-1 do
          table.insert(positions, console:GetSelectionNCaret(s))
        end
        -- process all selections from last to first
        table.sort(positions)
        local mainpos = console:GetSelectionNCaret(console:GetMainSelection())

        console:BeginUndoAction()
        for s = #positions, 1, -1 do
          local pos = positions[s]
          local startpos = console:WordStartPosition(pos, true)
          console:SetSelection(startpos, pos)
          console:ReplaceSelection(text)
          -- if this is the main position, save new cursor position to restore
          if pos == mainpos then mainpos = console:GetCurrentPos()
          elseif pos < mainpos then
            -- adjust main position as earlier changes may affect it
            mainpos = mainpos + #text - (pos - startpos)
          end
        end
        console:EndUndoAction()

        console:GotoPos(mainpos)
      else
        local pos = console:GetCurrentPos()
        local startpos = console:WordStartPosition(pos, true)
        local endpos = console:WordEndPosition(pos, true)
        console:SetSelection(startpos, ide.config.acandtip.droprest and endpos or pos)
        console:ReplaceSelection(event:GetText())
      end
    end)
...
if ok and rl.complete then
    rl.setup()
    rlcomplete = rl.complete
    debug.setfenv(rlcomplete,env)
else
    rlcomplete = require('lua-complete.rlcompleter').complete
    setfenv(rlcomplete,env)
end
  ...
  function (event)
    ...
    while true do
      ...
      if key == wx.WXK_UP or key == wx.WXK_NUMPAD_UP then
        if console:AutoCompActive() then break end
      ...
      elseif key == wx.WXK_TAB then
        if console:AutoCompActive() then
          console:AutoCompComplete()
          return
        end
        ...
        elseif not remotesend then
          table.sort(list)
          console:UserListShow(1,table.concat(list,' '))
          --console:AutoCompShow(#word,table.concat(list,' '))
        end
        return
