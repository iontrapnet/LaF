...
local LuaComplete = {
  Port = 51371,
  Client = require'lua-complete.client',
  Command = 'lua-complete'..(require('ffi').arch == 'x64' and '64' or '')..'.cmd',
}
local function LuaCompleteList(editor,pos)
  local filename = ide:GetDocument(editor):GetFileName()
  local content = editor:GetTextRange(0,pos)
  local response = LuaComplete.Client.sendRequest(filename, content, pos, '', LuaComplete.Port)
  if not response then
    --LuaComplete.Client.shutdown(LuaComplete.Port)
    os.execute(LuaComplete.Command..' client -x -p '..LuaComplete.Port)
    os.execute('hidec '..LuaComplete.Command..' server -p '..LuaComplete.Port)
  end
  response = LuaComplete.Client.sendRequest(filename, content, pos, {}, LuaComplete.Port)
