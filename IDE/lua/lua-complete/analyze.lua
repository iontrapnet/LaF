...
local function findEquals(fullTable, output, line)
    ...
    elseif t.tag  == "Call" and t.first_token == "require" and t[2].tag == "String" then
        output.modules[t[2][1]] = {
            ["module"] = t[2][1],
            ["location"] = line
        }
    end
...
local function analyzeTable(t)
    ...
    for k, v in pairs(t) do
        ...
        if type(k) == 'string' and k:sub(1,1) ~= '_' then
          ...
          if k ~= "__index" and valueType == "table" and v ~= t then
...
function analyze.analyzeModule(module, packagePath)
    ...
    if type(packagePath) == 'string' then
    ...
    -- go in for more info
    if analyzedModule == true then
        analyzedModule = loadstring('return '..module)()
    end
