...
local function getCursorVariable(src, cursor)
    ...
    if autoCompleteTableChars[currentPos] or autoCompleteFunctionChars[currentPos] then
        ...
        local start = stringReverseFind(src, tonumber(cursor), wordBoundarySet)
        return string.sub(src, start, cursor), nil
    end
end

local function parseCursorVariable(src, cursor)
    local cursorVariable, char = getCursorVariable(src, cursor)
    print('cursor', cursorVariable, char and string.char(char))
...
local function processRequest(line)
    ...
    elseif fileCache[filename][cursorVariable] then
        print("found in file cache",fileCache[filename][cursorVariable].name)
    ...
    if not cursorLookupModule then
        local output = {["type"] = "table", ["info"] = {}}
        local line
        if lastChar then
            line = cursorVariable..string.char(lastChar)
        else
            line = table.concat(cursorVariables,'.')
        end
        local list = rlcomplete(line)
        for _, v in ipairs(list) do
            ...
            table.insert(output.info,{name=v,type=t})
        end
        return output
    end
    
    -- traverse to the lastTable
    local lastTable = cursorLookupModule
    for i = 2,#cursorVariables do 
      local key = cursorVariables[i]
      local val = lastTable.table[k]
      if not val then
        val = {type = lastTable.type,table = {}}
        table.foreach(lastTable.table,function(k,v)
    ...
    if lastTable.type == "table" and (not lastChar or autoCompleteTableChars[lastChar]) then
