...
table.insert(M.completers.value, function(t, sep)
  ...
  for k, v in pairs(t) do
    ...
    --elseif type(k) == "number" and sep == "[" then
    --  coyield(k..']')
    end
  end
end)
...
function M.complete(word, line, startpos, endpos)
  if not line then
    return M.complete(word:match('^.*[%.:%[](.*)$') or word,word,#word,#word)
  end
  -- Helper function registering possible completion words, verifying matches.
  local matches = {}
  local function add(value)
    if value:sub(1,#word) == word then
  ...
  local function add_globals()
    ...
    call_completors(M.completers.value, getfenv() or _G)
  end
  ...
  local function contextual_list(expr, sep, str)
    ...
    if expr and expr ~= "" then
      ...
      if v then
        setfenv(v, getfenv() or _G)
        call_completors(M.completers.value, v(), sep)
      end
    end
    if #matches == 0 and not sep then
      add_globals()
    end
  end
