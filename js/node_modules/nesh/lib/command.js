...
(function() {
  var argv, config, disabled, enabled, exec, fs, install, isJs, nesh, optimist, opts, path, prefix, uninstall, _,
  ...
  optimist = require('optimist').usage('$0 [options]').options('c', {
  ...
  if (argv.enable) {
    ...
    config.pluginsExclude = _(config.pluginsExclude).reject(function(item) {
      return __indexOf.call(enabled, item) >= 0;
    });
    nesh.config.save();
  }
  ...
  if (argv.disable) {
    ...
    config.plugins = _(config.plugins).reject(function(item) {
      return __indexOf.call(disabled, item) >= 0;
    });
    if (config.pluginsExclude == null) {
      config.pluginsExclude = [];
    }
